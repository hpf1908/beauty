$(function(){
    ...
    $container.imagesLoaded(function(){
        ...
        $container.find('.item').animate({ opacity: 1 });
        $container.masonry({
            singleMode      : true,
            columnWidth     : 240,
            itemSelector    : '.item',
    ...
    var shareToWeibo = function(e){
        ...
        var shareUrl     = encodeURIComponent('http://'+ config.domain + '/show/' + itemid);
        ...
        var url          = 'http://v.t.sina.com.cn/share/share.php?appkey=' + config.weibo_api_key +
            '&url='      + shareUrl  +
            '&title='    + shareText +
            '&pic='      + shareImg  +
            '&ralateUid='+ ralateUid +
            '&source=&sourceUrl&content=utf-8';
        $.post('http://'+ config.domain + '/api/dolike',{itemid:itemid});
        var share = function(){
        ...
        WB2.login(function(e){
            WB2.anyWhere(function(W){
                W.widget.connectButton({
                    ...
                    callback : {
                        login:function(o){
                            SetCookie('userid', o.id);
                            SetCookie('nickname', o.screen_name);
                            location.reload();
                        },
                        logout:function(){
                            SetCookie('userid', '');
    ...
    if(nickname && userid){
        $(".topbar .weibo_login").replaceWith('<span>欢迎您'+nickname+'</span><span><a href="#" id="logout">退出</a></span>');
    }
